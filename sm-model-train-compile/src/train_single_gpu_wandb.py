...
def parser_args(train_notebook=False):
    ...
    parser.add_argument('--wandb_project', type=str, default='sm-training')
...
if __name__ == '__main__':
    ...
    if args.wandb_api_key is None:
        logger.info('Please setup your wandb api key first.')
    else:
        ### Initialize wandb
        wandb.login(key=args.wandb_api_key)
        run = wandb.init(project=args.wandb_project, reinit=True, resume=False, config=vars(args))    
        ...
        filename = os.path.join(args.model_dir, 'model_best.pth')

        ### Save wandb    
        wandb.save(filename)
        #model_artifact = wandb.Artifact('trained-imgcls', type='model', metadata=vars(args))
